...
void get_matrix(t_loop_data *mlx, t_main m)
{
	...
	mlx->model = Matrix4x4_mul(matrix4x4_set_translation((t_point){ abs(m.x_min) * scale + 30, (abs(m.y_min) * scale) + 30, 0.0f, 0}), Matrix4x4_mul(rotation, matrix4x4_set_scale((t_point){scale, scale, scale, 1.0f})));
	mlx->pers = pers;
}
